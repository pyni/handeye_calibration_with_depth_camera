...
sr[2] = (char*)"/home/yuan/doc/objectpose/qr_code_icp/src/calibblock.obj";  
...
grid_search.setInputCloud (object_xyz);
...
tf::StampedTransform transform_aru_tf;
  ...
  while (node.ok()){
    ...
    try{
      listener.lookupTransform("/aruco_marker_frame", "/camera_depth_optical_frame",  
                               ros::Time(0), transform_aru_tf); 
      break;
    }
...
transform_trans_humanpredefined(0,3)= 0.1;//这个应该是二维码相对于物体坐标系的变换,先移动

Eigen::Matrix4f transform_rotation_humanpredefined = Eigen::Matrix4f::Identity();  //这个应该是二维码相对于物体坐标系的变换,再绕自己转动

 Eigen::AngleAxisf rollAngle(0, Eigen::Vector3f::UnitX());
Eigen::AngleAxisf pitchAngle(0.78539816339745 , Eigen::Vector3f::UnitY());
...
transform_aru=transform_rotation_humanpredefined*transform_trans_humanpredefined*transform_aru;

Eigen::Matrix4f transform_aru_inv =transform_aru.inverse(); 
pcl::transformPointCloud (*object_xyz, *object_xyz,transform_aru_inv);  
...
transformationfinal=transformation2.inverse()*unittranform ;//因为之前ＩＣＰ写成了环境匹配鞋子，而不是鞋子匹配环境，所以要逆
